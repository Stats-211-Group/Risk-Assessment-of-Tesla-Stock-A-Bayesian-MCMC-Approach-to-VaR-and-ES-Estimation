...
def main():
    ...
    nu_med = float(2.0 + np.exp(np.median(post[:,2])))
        # ======（添加开始）Posterior histograms for (mu, sigma, nu) ======
    mu_samps = post[:, 0]
    sigma_samps = np.exp(post[:, 1])         # sigma = exp(log_sigma)
    nu_samps = 2.0 + np.exp(post[:, 2])      # nu   = 2 + exp(eta)
    ...
    # 画图 + 保存
    plot_hist(mu_samps,    "Posterior of μ", "mu (%)",    "/mnt/data/post_mu_hist.png")
    plot_hist(sigma_samps, "Posterior of σ", "sigma (%)", "/mnt/data/post_sigma_hist.png")
    plot_hist(nu_samps,    "Posterior of ν", "nu (df)",   "/mnt/data/post_nu_hist.png")

    # 可选：Trace plots（混合与收敛直观检查）
    ...
    plot_trace(mu_samps,          "Trace: μ", "mu (%)",            "/mnt/data/trace_mu.png")
    plot_trace(sigma_samps,       "Trace: σ", "sigma (%)",         "/mnt/data/trace_sigma.png")
    plot_trace(nu_samps,          "Trace: ν", "nu (df)",           "/mnt/data/trace_nu.png")

    # 可选：打印中位数和95%区间，方便图注或报告
    def summary(x):
        return np.median(x), np.percentile(x, 2.5), np.percentile(x, 97.5)

    print("\n=== Posterior histograms saved ===")
    print(" /mnt/data/post_mu_hist.png")
    print(" /mnt/data/post_sigma_hist.png")
    print(" /mnt/data/post_nu_hist.png")
    print("Trace plots: /mnt/data/trace_mu.png, /mnt/data/trace_sigma.png, /mnt/data/trace_nu.png")

    print("\nQuick summaries [median, 2.5%, 97.5%]:")
    print(" mu   :", summary(mu_samps))
    print(" sigma:", summary(sigma_samps))
    print(" nu   :", summary(nu_samps))
    # ======（添加结束）=====
